...
class Vader extends Component {
  ...
  componentDidMount() {
    ...
    const socket = window.io.connect(`${appConfig.apiUrl}`, {path: '/asr', withCredentials: true});

    console.log(`connecting to ${appConfig.apiUrl}`);
  ...
  renderMain() {
    return (
      <div
        className="m-0 d-flex w-100 h-100 flex-column"
        style={{visibility: this.state.isAuthenticated ? 'visible' : 'hidden'}}>
        <div className="m-0 w-100 overflow-hidden" style={{flexGrow: 1}}>
          {this.state.isAuthenticated && (
            <video
              src="https://d2iia7yeg2usc9.cloudfront.net/Darth_Vader.mp4"
        ...
        <div
          className="audio-control d-flex align-items-center"
          style={{height: '30px', margin: '20px 20px 10px 20px'}}>
          <audio></audio>
          <button id="startRecButton" type="button">
            Start
          </button>
          <button id="stopRecButton" type="button">
            Stop
          </button>
